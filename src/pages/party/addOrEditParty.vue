<template>
    <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="100">
        ...
        <FormItem label="浏览次数" prop="viewTimes">
            <InputNumber :min="0" v-model="formValidate.viewTimes" placeholder="浏览次数" />
        </FormItem>
        <FormItem label="是否置顶">
  ...
  methods: {
    handleSubmit (name) {
      this.$refs[name].validate((valid) => {
        if (valid) {
          this.$emit('addParty', Object.assign({}, this.editParty, this.formValidate, {isPush: +(!!this.formValidate.isPush)}))
        } else {
